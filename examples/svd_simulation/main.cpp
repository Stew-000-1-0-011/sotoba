...
int main() {
	...
	Vec3 tikhnov{};
	std::cin >> loop_num >> accept_distance;
	std::cin >> tikhnov.x() >> tikhnov.y() >> tikhnov.z();
	...
	// LiDAR作成
	const auto lidar = sim::RosettaLidar::RosettaLidar::make(sim::mid360(10));
	// 乱数生成器
	...
	// icp_resourceの作成
	auto icp = icp_resource::to_resource<icp_resource::SvdNonSyclResource>(lidar.get_points_num(), std::span{objects});
	...
	auto update = [&](const my_pango_util::KeyboardHandler& keys) {
		control_duration_timer.sleep_for(100ms);
		bench_timer.clear();
		// 入力を処理
		{
			...
			constexpr float trans_speed = 1.0f;
			constexpr float rot_speed = 0.5 * std::numbers::pi;
