...
namespace sotoba::surface::box_impl {
	...
	struct BoxOuter final {
		...
		BoxOuter(const Vec3& center, const SquareMat<3>& rot, const Vec3& hlens, const std::array<bool, 6> wall_not_exist = {}) noexcept
		: center{center}
		, wall_exist{}
		, rot{rot}
		, hlens{hlens}
		...
		auto closest_pdn(const Vec3& p) const noexcept -> std::pair<Vec4, UVec3> {
			...
			return {
				{this->center + this->rot.transpose() * cq_local, Vec{d}}
				, n
			};
		}
		...
		auto ray_collision(const UVec3& ray) const noexcept -> float {
			...
			for(u8 i = 0; i < 3; ++i) {
				...
				if(math::fabs(ray_d) < math::epsilon){
					if(!(-hlens[i] < co_local[i] && co_local[i] < hlens[i])) return std::numeric_limits<float>::infinity();
					else continue;
				}
