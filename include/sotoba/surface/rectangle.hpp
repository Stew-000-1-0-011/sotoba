...
namespace sotoba::surface::rectangle_impl {
	...
	struct Rectangle final {
		...
		Rectangle(
			const Vec3& center,
			const Vec4& u_axis_and_hlen,
			const Vec4& v_axis_and_hlen,
			const UVec3& normal
		) noexcept
			: center{center}
			, dummy1{0.f}
		...
		auto ray_collision(const UVec3& ray) const noexcept -> float {
			...
			// レイが平面と平行な場合 (内積がほぼ0)
			if (math::fabs(denom) < epsilon) { return std::numeric_limits<float>::infinity(); }
...
TEST_SUITE("rectangle.hpp") {
	...
	TEST_CASE("ray_collision: Intersection Test (Ray from Origin)") {
		...
		SUBCASE("Hit (Inside Bounds)") {
			...
			CHECK(
				ApproxCheck{Vec{ray_dir} * math::sqrt(res)} == ApproxCheck{Vec3{1.5f, 0.5f, 5.f}}
			);
		}
	...
	TEST_CASE("Rectangle::apply_se3") {
		...
		SUBCASE("Translation only") {
			...
			// 方向ベクトルとサイズは変化しないはず
			CHECK(
				ApproxCheck{Vec3{rect.u_axis_and_hlen.xyz()}} == ApproxCheck{Vec3{1.0f, 0.0f, 0.0f}}
			); // U = X
			CHECK(
				ApproxCheck{Vec3{rect.v_axis_and_hlen.xyz()}} == ApproxCheck{Vec3{0.0f, 1.0f, 0.0f}}
			); // V = Y
			CHECK(
				ApproxCheck{UVec3{rect.normal}} == ApproxCheck{UVec3{0.0f, 0.0f, -1.0f}}
			); // Normal = -Z
		...
		SUBCASE("Rotation only (90 degrees around Y-axis)") {
			...
			CHECK(
				ApproxCheck{Vec3{rect.u_axis_and_hlen.xyz()}}
				== ApproxCheck{Vec3{0.0f, 0.0f, -1.0f}}
			);
		...
		SUBCASE("Identity Transformation") {
			...
			CHECK(
				ApproxCheck{Vec3{rect.u_axis_and_hlen.xyz()}}
				== ApproxCheck{Vec3{base_rect.u_axis_and_hlen.xyz()}}
			);
