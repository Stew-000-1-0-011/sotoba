...
namespace sotoba::sim::rosetta_lidar_impl {
	...
	struct RosettaLidar final {
		...
		static auto make(const RosettaLidarConfig& conf) noexcept -> RosettaLidar {
			...
			const float el_center =
				(conf.elevation.first + conf.elevation.second) / 2.f * deg_to_rad;
	...
	inline constexpr auto mid360(const u32 scan_hz, const float down_sampling_rate = 1.f)
		-> RosettaLidarConfig {
	...
	inline constexpr auto sphere(const u32 scan_hz, const float down_sampling_rate = 1.f)
		-> RosettaLidarConfig {
		return RosettaLidarConfig{
			.elevation = {-90.f, 90.f},
			.azimuth = {-180.f, 180.f},
