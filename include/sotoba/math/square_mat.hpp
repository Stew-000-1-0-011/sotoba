...
namespace sotoba::math {
	...
	struct SquareMat final {
		...
		constexpr friend auto operator+(const SquareMat& lhs, const SquareMat& rhs) noexcept -> SquareMat {
			SquareMat ret = lhs;
			ret += rhs;
			return ret;
		}
		...
		constexpr friend auto operator*(const float lhs, const SquareMat& rhs) noexcept -> SquareMat {
			SquareMat ret = rhs;
			ret *= lhs;
			return ret;
		}

		constexpr friend auto operator*(const SquareMat& lhs, const float rhs) noexcept -> SquareMat {
			SquareMat ret = lhs;
			ret *= rhs;
			return ret;
		}
		...
		constexpr friend auto operator*(const SquareMat& lhs, const Vec<d_, is_unit_>& rhs) noexcept -> Vec<d_, false>
		#else
		constexpr friend auto operator*(const SquareMat& lhs, const Vec<d_>& rhs) noexcept -> Vec<d_, false>
		#endif
		{
			Vec<d_, false> ret{};
			for(u8 i = 0; i < d_; ++i) for(u8 j = 0; j < d_; ++j) {
				ret[i] += lhs[i, j] * rhs[j];
			}
			return ret;
		}
		...
		constexpr auto transpose() const noexcept -> SquareMat {
			SquareMat ret = *this;
			for(u8 i = 0; i < d_; ++i) for(u8 j = i + 1; j < d_; ++j) {
...
namespace sotoba::math {
	...
	struct ApproxCheckImpl<SquareMat<n_>> final {
		static auto compare(const SquareMat<n_>& m1, const SquareMat<n_>& m2, const std::optional<float> eps = 1.e-4f) -> bool {
			for(u8 i = 0; i < n_; ++i) for(u8 j = 0; j < n_; ++j) {
				if(m1[i, j] != (eps ? doctest::Approx(m2[i, j]).epsilon(*eps) : doctest::Approx(m2[i, j]))) return false;
			}
			return true;
		}
	};
