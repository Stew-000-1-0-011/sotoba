...
namespace sotoba::math {
	...
	struct ApproxCheckImpl<math::SymMat<n_>> final {
		static auto compare(const SymMat<n_>& m1, const SymMat<n_>& m2, const std::optional<float> eps = 1.e-4f) -> bool {
			for(u8 i = 0; i < n_ * (n_ + 1) / 2; ++i) {
				if(m1.v[i] != (eps ? doctest::Approx(m2.v[i]).epsilon(*eps) : doctest::Approx(m2.v[i]))) return false;
			}
			return true;
		}
	};
