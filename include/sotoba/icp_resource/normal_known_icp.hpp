			...
			for(u32 iloop = 0; iloop < loop_num; ++iloop) {
				...
				[&]<usize ... idxs_>(std::index_sequence<idxs_ ...>) {
					for(usize ip = 0; ip < point_cloud.size(); ++ip) {
						...
						std::pair<std::pair<Vec4, UVec3>, ObjSurfId> q{{{Vec3{}, Vec{std::numeric_limits<float>::infinity()}}, {}}, ObjSurfId::Null};
				...
				for(u8 iobj = 0; iobj < this->obj_num; ++iobj) {
					...
					const SE3 diff = SE3 {
						math::UQuaternion{vec::fast_normalize(Vec4{x[0], x[1], x[2], 2.f})}
						, {x[3], x[4], x[5]}
					};
