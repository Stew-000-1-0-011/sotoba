...
namespace sotoba::icp_resource::resource_impl {
	...
	concept icp_resource = (surfacelike<Ss_> && ...) && requires (
		T_ mut
		, const T_ imut
	...
	inline auto to_resource(const usize points_num, std::span<const std::span<const std::variant<Ss_ ...>>>&& objects) -> Resource_<Ss_ ...> {
